...
class PurchasesControllerTest {
    ...
    @Test
    public void testPurchaseCurrencyCodeDE() throws Exception {
        String body = getPurchaseJson("2", "customer 2", "2020-01-01T10:00:00+01:00", 25.34, "DE", "product2");
        mockMvc.perform(post("/purchases")
                .contentType(MediaType.APPLICATION_JSON).content(body)
        ).andExpect(status().isOk());
    ...
    public void testPurchaseStatistics() {
        ...
        purchasesRepository.save(new Purchase("1", now.minusDays(20), Collections.emptyList(), "", 10.0));
//        purchasesRepository.save(new Purchase("1", now.minusDays(20), Collections.emptyList(), "", 0.0));
